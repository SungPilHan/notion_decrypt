...
async function sqliteReadWrite({ connection, sql, args, queryName, }) {
    const statement = {
        sql,
        args,
        getData: true,
    };
    const [result] = await executeSqliteTransaction({
        connection,
...
async function sqliteWrite({ connection, sql, args, queryName, }) {
    const statement = {
        sql,
        args,
    };
    await executeSqliteTransaction({
        connection,
        statements: [statement],
        queryName,
    });
...
async function executeSqliteTransaction({ connection, statements, queryName, }) {
    const executeSqliteTransactionInner = async () => {
        ...
        const trimmed = statements.map(s => {
            return Object.assign(Object.assign({}, s), { sql: (0, utils_1.trimQuery)(s.sql) });
        });
        const body = [begin, ...trimmed, commit];
        const batch = (0, sqliteTypes_1.makeSqliteBatch)({
            queryName,
            body,
            onError: rollback,
        });
...
function makeSqliteException(batch, result) {
    ...
    switch (errorResult.type) {
        ...
        case "OutOfSpace":
            ...
            break;
        default:
            (0, typeUtils_1.unreachable)(errorResult);
    }
