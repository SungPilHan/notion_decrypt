...
class SqliteConnectionWrapper {
    ...
    execSqliteBatchV2(args) {
        ...
        if (precondition) {
            ...
            if (preconditionResult.type === "DataOk") {
                const { precondition_result } = (0, sqliteHelpers_1.getSingleRowResultAsType)(preconditionResult);
    ...
    execSqliteBatchInternal(batch) {
        ...
        for (const [index, statement] of body.entries()) {
            ...
            errored = (0, sqliteTypes_1.isSqliteError)(result);
        }
        return {
            body: results,
    ...
    execSqliteStatement(statement) {
        try {
            ...
            if (getData) {
                const result = transformNativeModuleData(preparedStatement.reader
...
function transformNativeModuleData(data) {
